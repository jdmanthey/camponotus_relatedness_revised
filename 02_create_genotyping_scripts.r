...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/12_relatedness"
	...
	popmap <- "camp_popmap.txt"
	individuals <- read.table(popmap, sep="\t")
	faidx <- read.table("camp_sp_genome_filtered.fasta.fai", stringsAsFactors=F)
	...
	for(a in 1:nrow(individuals)) {
		if(a == 1) {
			helper1 <- cbind(faidx[,1], as.character(faidx[,3]), rep(as.character(individuals[a,1]), nrow(faidx)))
		} else {
			helper1 <- rbind(helper1, cbind(faidx[,1], as.character(faidx[,3]), rep(as.character(individuals[a,1]), nrow(faidx))))
		}
	}
